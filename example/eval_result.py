...
class EvalResult:
    def __init__(self, graph_path: str, subgraph_deque: Deque[Subgraph]):
        ...
        self.subgraph_size = sum(len(g.nodes[0]) for g in subgraph_deque)
    ...
    def MDL(self) -> float:
        ...
        return self.graph_size / (self.subgraph_size + self.new_graph_size)
    ...
    def reduce_ratio(self) -> float:
        return (self.graph_size - self.subgraph_size - self.new_graph_size) / self.graph_size
